...
const raceGoalPresets = ["5K", "10K", "Half Marathon", "Marathon"] as const;
const nonRaceGoalPresets = ["Base Building", "Recovery"] as const;
...
const distanceUnitOptions = ["meters", "kilometers", "miles"] as const;
const surfaceOptions = ["road", "track", "trail", "treadmill", "mixed"] as const;
const interruptionOptions = ["injury", "illness", "life", "travel"] as const;
const effortModifierOptions = [
  "pushedStroller",
  "ranWithDog",
  "trailOffRoad",
  "treadmill",
...
type StrengthEquipmentOption = (typeof strengthEquipmentOptions)[number];
type DistanceUnitOption = (typeof distanceUnitOptions)[number];
type SurfaceOption = (typeof surfaceOptions)[number];
type InterruptionOption = (typeof interruptionOptions)[number];
type EffortModifierOption = (typeof effortModifierOptions)[number];
...
function formatWeekdayShort(day: Weekday) {
...
function formatDurationInput(value?: number | null) {
...
function toMeters(value: string, unit: DistanceUnitOption) {
...
function fromMeters(value: number, unit: DistanceUnitOption) {
  ...
  switch (unit) {
    ...
    case "miles":
      return (value / 1609.344).toFixed(2).replace(/\.00$/, "");
    default:
      return "";
  }
}

function toggleArrayValue<T extends string>(value: T, current: T[]) {
  return current.includes(value)
    ? current.filter((entry) => entry !== value)
    : [...current, value];
}

function clampRunningDaysPerWeek(value: number, selectedDays: Weekday[]) {
    ...
    <BrowserRouter>
      <Routes>
        ...
        >
          ...
          <Route
            path="/plan/workout/:workoutId"
            element={<WorkoutPage session={session} />}
          />
          ...
          <Route
            path="/settings"
            element={<SettingsPage onRefresh={refresh} session={session} />}
          />
        </Route>
...
function OnboardingPage({
  ...
  const [customPersonality, setCustomPersonality] = useState(
    session.personality.name === "custom"
      ? session.personality.description
      : "",
  );
  ...
  return (
    <main className="onboarding-shell">
      <Screen
        title="Onboarding"
        subtitle={`Step ${currentStepNumber} of 10`}
      >
            ...
            <div className="form-grid">
              <Field label="Run days per week">
                <input
                  ...
                  min={1}
                  onChange={(event) =>
                    setRunningDaysPerWeek(Number(event.target.value))
                  }
                  type="number"
              ...
              <Field label="Preferred long run day">
                <select
                  onChange={(event) =>
                    setLongRunDay(event.target.value as Weekday | "")
                  }
                  value={longRunDay}
                >
...
function PlanPage({
  ...
  const [interruptionType, setInterruptionType] =
    useState<InterruptionOption>("life");
  ...
  useEffect(() => {
    ...
    setGoalEditType(
      currentPlan.goalType === "race" ? "race" : "nonRace",
    );
    ...
    <Card
      ...
      actions={
        hasExistingPlanWork ? (
          <Button
            kind="secondary"
            onClick={() => setShowCreateDraft((current) => !current)}
          >
          ...
          <>
            ...
            <div className="form-grid">
              <Field label="Goal type">
                <select
                  onChange={(event) =>
                    setGoalType(event.target.value as "race" | "nonRace")
                  }
                  value={goalType}
                >
                ...
                !latestProposal.consumedByPlanId ? (
                  <div className="stack">
                    ...
                    >
                      <input
                        onChange={(event) =>
                          setProposalPeakWeekVolume(event.target.value)
                        }
                        type="number"
        ...
        <div className="stack">
          ...
          <div className="metric-list wrap">
            {currentPlan.weeks.map((week) => (
              ...
              >
                W{week.weekNumber} · {Math.round(week.targetVolumePercent * 100)}%
                {" · "}
                {week.emphasis}
              </Link>
            ))}
                  ...
                  <div className="form-grid">
                    <Field label="Why this week needs a pause">
                      <select
                        onChange={(event) =>
                          setInterruptionType(
                            event.target.value as InterruptionOption,
                          )
                        }
                      ...
                      void run(
                        async () =>
                          reportPlanInterruption({
                            ...
                            note:
                              interruptionNote.trim() ||
                              "Marked from web companion.",
                          }),
                ...
                <Field label="Distance">
                  <div className="inline-field">
                    ...
                    <select
                      onChange={(event) =>
                        setRaceDistanceUnit(
                          event.target.value as DistanceUnitOption,
                        )
                      }
            ...
            {planView.pastPlans.map((plan) => (
              <div className="row-card" key={String(plan._id)}>
                <div>
                  ...
                  <div>
                    {formatFriendlyLabel(plan.status)}
                    {" · "}
          ...
          >
            <div className="stack">
              <p>
                ...
                {formatDateKey(week.week.weekEndDateKey)}
                {" · "}
                target{" "}
                {formatVolume(
  ...
  useEffect(() => {
    ...
    if (detail.executionDetail) {
      ...
      setCustomModifierText(
        detail.executionDetail.execution.customModifierText ?? "",
      );
    } else {
      setNotes("");
      setSelectedModifiers([]);
                ...
                <div className="stack">
                  <div className="inset">
                    ...
                    <p>
                      ...
                      {" · "}
                      {formatDuration(
                        detail.executionDetail.importedWorkout.durationSeconds,
                      )}
                      {" · pace "}
                      {formatPace(
                      ...
                      void run(
                        async () =>
                          submitCheckIn({
                            ...
                            customModifierText:
                              customModifierText.trim() || undefined,
                            notes,
                          }),
                        "Check-in submitted.",
                      )
        ...
        <>
          <div className="two-column">
            <Card title="Filters" eyebrow="Feed">
              <div className="pill-row wrap">
                {(["all", "matched", "needsReview", "unplanned"] as const).map(
                  (value) => (
                    <button
                      key={value}
                      className={cx(
                        "pill-button",
                        filter === value && "pill-button-active",
                      )}
            ...
            <Card title="Priority" eyebrow="Fix first">
              {firstNeedsReview ? (
                <div className="stack">
                  ...
                  <ActionLink
                    to={`/history/${String(firstNeedsReview._id)}`}
                  >
                  ...
                  >
                    <div>
                      ...
                      <div>
                        ...
                        {" · pace "}
                        {formatPace(
                          workout.rawPaceSecondsPerMeter,
                          session.user.unitPreference,
                        )}
                      </div>
                    </div>
          ...
          <Card title={formatDateTime(detail.workout.startedAt)} eyebrow="Run summary">
            <div className="stack">
              <p>
                ...
                {" · pace "}
                {formatPace(
                  detail.workout.rawPaceSecondsPerMeter,
                  session.user.unitPreference,
                )}
              </p>
              <div className="button-row wrap">
...
function SettingsPage({
  ...
  const [competitiveness, setCompetitiveness] =
    useState<CompetitivenessLevel>("balanced");
  const [personality, setPersonality] =
    useState<PersonalityPreset>("noNonsense");
  const [customPersonality, setCustomPersonality] = useState("");
  ...
  const [courseDistanceUnit, setCourseDistanceUnit] =
    useState<DistanceUnitOption>("miles");
  ...
  useEffect(() => {
    ...
    setVolumePreference(
      (settings.user?.volumePreference ?? "time") as VolumeMode,
    );
    ...
    setLongRunDay(
      (settings.runningSchedule?.preferredLongRunDay ?? "") as Weekday | "",
    );
    setQualityDays(settings.runningSchedule?.preferredQualityDays ?? []);
    setCompetitiveness(
      (settings.competitiveness?.level ?? "balanced") as CompetitivenessLevel,
    );
    setPersonality(
      (settings.personality?.name ?? "noNonsense") as PersonalityPreset,
    );
    ...
    setStrengthEquipment(
      settings.strengthPreference.equipment as StrengthEquipmentOption[],
    );
  }, [
  ...
  const saveCoaching = async () => {
    ...
    if (
      ...
      await updatePersonality({
        preset: personality,
        customDescription:
          personality === "custom" ? customPersonality.trim() : undefined,
      });
    }
  ...
  const downloadExport = () => {
    ...
    anchor.href = url;
    anchor.download = `slopmiles-export-${new Date()
      .toISOString()
      .slice(0, 10)}.json`;
    anchor.click();
              ...
              <div className="stack">
                ...
                <div className="form-grid">
                  ...
                  <Field label="Surface">
                    <select
                      onChange={(event) =>
                        setCourseSurface(event.target.value as SurfaceOption)
                      }
                      value={courseSurface}
                    >
            ...
            <div className="stack">
              <div className="button-row wrap">
                ...
                <Button
                  kind="danger"
                  onClick={() =>
                    void run(async () => resetAppData({}), "App data reset.")
                  }
                >
                  Reset app
                </Button>
