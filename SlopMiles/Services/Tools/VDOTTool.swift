...
struct VDOTTool {
    static func calculateVDOT(raceDistanceMeters: Double, raceTimeSeconds: Double) -> [String: JSONValue] {
        ...
        return ["vdot": .number(round(vdot.value * 10) / 10)]
    }

    static func getTrainingPaces(vdot vdotValue: Double) -> [String: JSONValue] {
        ...
        for (name, intensity) in intensities {
            ...
            paces["\(name)_min_per_km"] = .number(round(minPerKm * 100) / 100)
            paces["\(name)_formatted"] = .string(formatPace(minPerKm))
        }
        return paces
    }

    static func projectRaceTime(vdot vdotValue: Double, distanceMeters: Double) -> [String: JSONValue] {
        let v = Vdot(value: vdotValue)
        let distance = Measurement<UnitLength>(value: distanceMeters, unit: .meters)
        let seconds = v.projectedRaceTime(distance: distance)
        return [
            "projected_time_seconds": .number(round(seconds * 10) / 10),
            "projected_time_formatted": .string(formatDuration(seconds)),
        ]
    }
