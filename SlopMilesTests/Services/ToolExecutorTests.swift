...
struct ToolExecutorTests {
    ...
    func routesVDOT() async {
        let call = ToolCall(id: "1", name: "calculate_vdot", arguments: [
            "race_distance_meters": 5000.0, "race_time_seconds": 1200.0,
        ])
    ...
    func routesPaces() async {
        let call = ToolCall(id: "2", name: "get_training_paces", arguments: ["vdot": 50.0])
        let result = await executor.execute(call)
    ...
    func routesHRZones() async {
        let call = ToolCall(id: "3", name: "calculate_hr_zones", arguments: ["max_hr": 190])
        let result = await executor.execute(call)
    ...
    func routesPaceConvert() async {
        let call = ToolCall(id: "4", name: "convert_pace", arguments: [
            "value": 5.0, "from_unit": "min_per_km", "to_unit": "min_per_mile",
        ])
        let result = await executor.execute(call)
    ...
    func routesMileage() async {
        let call = ToolCall(id: "5", name: "check_mileage_progression", arguments: [
            "weekly_distances_km": [30.0, 33.0, 36.0],
        ])
        let result = await executor.execute(call)
    ...
    func executeAll() async {
        let calls = [
            ToolCall(id: "a", name: "calculate_vdot", arguments: ["race_distance_meters": 5000.0, "race_time_seconds": 1200.0]),
            ToolCall(id: "b", name: "get_training_paces", arguments: ["vdot": 50.0]),
        ]
